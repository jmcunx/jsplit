...
LFLAGS=$(WALL) $(LIBJLIB2) -o jsplit $(LIBS)

ALL_OBJ=jsplit$(OBJ) jsplit_b$(OBJ) jsplit_i$(OBJ) jsplit_u$(OBJ) jsplit_a$(OBJ) jsplit_h$(OBJ) jsplit_t$(OBJ)
ALL_C=jsplit.c jsplit_b.c jsplit_i.c jsplit_u.c jsplit_a.c jsplit_h.c jsplit_t.c
ALL_H=jsplit.h

#
# do the work
#
all:	ckenv tags $(ALL_OBJ) jsplit.1$(GZ)
...
jsplit.1.gz:	jsplit.1
	-$(RM) jsplit.1.gz
	$(GZIP) jsplit.1

jsplit.1:	jsplit.man
	$(NROFF) jsplit.man > jsplit.1
...
install:	all
	...
	-$(CHMOD) 755 $(PRODMAN)
	-$(STRIP) jsplit
	$(CP) jsplit $(PRODUCTION)/jsplit
	$(CHMOD) 755 $(PRODUCTION)/jsplit
	$(CP) jsplit.1$(GZ) $(PRODMAN)/jsplit.1$(GZ)
	$(CHMOD) 644 $(PRODMAN)/jsplit.1$(GZ)

uninstall:	all
	-$(RM) $(PRODUCTION)/jsplit
	-$(RM) $(PRODMAN)/jsplit.1.gz
	-$(RM) $(PRODMAN)/jsplit.1
